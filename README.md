...
- **Admin editor WYSIWYG parity:** the post editor ([`resources/views/admin/posts/_form.blade.php`](resources/views/admin/posts/_form.blade.php)) also loads `article.css` and wraps each locale's CKEditor instance in the same `.article-paper` container the public article view uses, so the editing surface's font, color, and heading treatment match the published post pixel-for-pixel instead of showing CKEditor's default black-on-white chrome. `article.css` additionally re-skins CKEditor's own UI chrome (toolbar, dropdowns, balloons, tooltips) via its `--ck-color-*` custom properties, scoped to `.article-paper`, so the whole editing panel — not just the article text — matches the active theme; CKEditor derives properties like `--ck-color-toolbar-background` from the base `--ck-color-base-*` vars only once at `:root`, so each derived token an actual UI element paints with is redeclared directly rather than relying on CKEditor's own indirection. Each locale also has a client-side-only **Preview** toggle that swaps the live editor for a read-only render using `blog/show.blade.php`'s exact markup (`.journal-hero`, `.article-image`, `.article-paper > .ck-content`), reading straight from the CKEditor instance (`getData()`) so unsaved edits show up too — no new backend route, no save required. Inline image text-wrap (float left/right around a paragraph) is CKEditor's built-in `imageStyle:wrapText` toolbar group — select an image, resize it with the drag handles so it isn't full-width, then choose an align-left/right style; the float is visible immediately while editing, not just after publish, since the theme CSS's `figure.image.image-style-*` float rules apply to the same `.ck-content` class CKEditor gives the editable region.
